...
{
  std::cout << "Skid Command VelocityLeft:" << velocityLeft << " VelocityRight:" << velocityRight << "\n";
...
void joyCallback(const sensor_msgs::Joy::ConstPtr& msg)
{
  ...
  if (msg->buttons[TOP_LEFT_BUTTON] > 0) {
    ...
    velocityL = msg->axes[LEFT_UP] * MAX_RAW_VELOCITY * 0.4;
    if (abs(msg->axes[LEFT_UP]) < DEADBAND)  {
      velocityL = 0;
      }
    velocityR = msg->axes[RIGHT_UP] * MAX_RAW_VELOCITY * 0.4;
    if (abs(msg->axes[RIGHT_UP]) < DEADBAND)  {
      velocityR = 0;
      }
  } else {
